...
namespace WeatherDataAnalysis.View
{
    ...
    public class ReportBuilder
    {
        ...
        {
            var days = this.data.FindDayHighestPrecipitationOccuredOn(year);

            this.Report += $"The highest precipitation of {days[FirstIndex].Precipitation:0.00} occured on the date(s): {string.Join(", ", days.Select(d => d.Date.ToShortDateString()))}" 
                           + Environment.NewLine;
        }
